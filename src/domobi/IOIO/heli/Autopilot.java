...
public class Autopilot {
  ...
  public float        kPr = 0.0f, kIr = 0.0f, kDr = 0.0f;
  ...
  public void setOrientation(JSONObject setpoint) throws JSONException {
    ...
    desiredOrientation.setYaw((float) yaw);
  ...
  public Helicopter getNextState(Odometry odom) {
    ...
    Orientation error = desiredOrientation.getDifference(odom.getOrientation());
    pitchError = error.pitch;
    yawError = error.getYaw();
    ...
    if (heliModel.getMainPwr() > 0.4f) {
      ...
      heliModel.setTailPwr(kPt * uPt + kIt * uIt + kDt * uDt);
      ...
      // heliModel.setRotationPwr((kPr*uPr+kIr*uIr+kDr*uDr));
    } else {
