...
public class Orientation {
		 ...
		 * 
		 * For now, using a 2D vector for "yaw" and angle for pitch...
		 * with absolute value for mainPwr. Eventually extend Orientation +
		 * localization for 4D quaternion, 3D (lat, long, alt) global location.
		 * 
		 * Pitch is the angle from level.
		 * 
		 * Yaw is location along the unit circle (x,y), kept as an angle, with
		 * methods for getting deltas...(to avoid gimbal lock).
		 * 
		...
		static float perpDotProduct(float[] a, float[] b)	{
			return (float)Math.atan2( a[0]*b[1] - a[1]*b[0], a[0]*b[0] + a[1]*b[1] );
		}
}
